...
if [ "$#" -ne 2 ]; then
    command="ffmpeg -re -i $inFile -vcodec hevc -acodec copy -r "$fps" -f rtsp -rtsp_transport tcp $outRTSPAddress"
else
    command="ffmpeg -re -i $inFile -vcodec hevc -acodec copy -f rtsp -rtsp_transport tcp $outRTSPAddress"
fi
